...
class BasketApplication(Application):
	...
	edit_view = get_class('basket.views', 'EditBasketLineView')
	line_remove_view = get_class('basket.views', 'LineRemoveView')
	...
	def get_urls(self):
		urls = [
			...
			url(r'^saved/$', login_required(self.saved_view.as_view()),
				name='saved'),
			url(r'^update/$', self.edit_view.as_view(), name='update'),
			url(r'^delete/$', self.line_remove_view.as_view(), name='line-remove'),
		]
