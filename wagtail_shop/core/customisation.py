...
def inherit_app_config(local_app_path, app_package, app_label):
    ...
    create_file(
        ...
        "from wagtail_shop.oscar_apps.{app_label} import config\n\n\n"
...
def fork_app(label, folder_path, logger=None):
    ...
    oscar_app_path = join(wagtail_shop.__path__[0], 'oscar_apps', label_folder)
    if exists(os.path.join(oscar_app_path, 'admin.py')):
        logger.info("Creating admin.py")
        create_file(join(local_app_path, 'admin.py'),
                    "from wagtail_shop.oscar_apps.%s.admin import *  # noqa\n" % label)
    ...
    if exists(oscar_models_path):
        ...
        #    "from wagtail_shop.oscar_apps.%s.models import *  # noqa isort:skip\n" % label)
        shutil.copyfile(oscar_models_path, join(local_app_path, 'models.py'))
